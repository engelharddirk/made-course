...
expected_category_name_df = pd.DataFrame(
    [[35,"Comedy","en"],
     [37,"Western","de"],
     [71,"Familiendrama","de"]],
     columns=["category_id","name","language_iso_639_1"]
)
expected_life_ladder_df = pd.DataFrame(
    [["Germany", 2015, 7.037],
     ["Germany", 2016, 6.874],
     ["Germany", 2017, 7.074]],
...
def test_data_content(dataframes):
    ...
    pd.testing.assert_frame_equal(dataframes[5], expected_life_ladder_df, check_dtype=False)
    pd.testing.assert_frame_equal(dataframes[3], expected_category_name_df, check_dtype=False)
